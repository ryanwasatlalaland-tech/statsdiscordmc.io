...
let modalHistoryPoints = [];
let modalHistoryRange = "7d";
  ...
  chart = new Chart(canvas,{
    ...
    data:{
      ...
      datasets:[{
        ...
        backgroundColor:visibleServers.map((_,index)=>index===0 ? "rgba(197,255,255,.86)" : "rgba(88,229,238,.68)"),
        borderColor:visibleServers.map((_,index)=>index===0 ? t.beacon : t.diamond),
        borderWidth:1,
        borderRadius:8
      }]
    },
    options:{
      ...
      scales:{
        x:{grid:{display:false},ticks:{color:t.text,font:{weight:"700"}}},
        y:{beginAtZero:comparisonMetric!=="growth24h",grid:{color:t.grid},ticks:{color:t.text,font:{weight:"700"},callback:value=>Number(value).toLocaleString(undefined,{notation:"compact",maximumFractionDigits:1})}}
      }
    }
  });
}
...
function topHistoryKey(server){
  return String(server?.id || server?.code || server?.slug || server?.name || "").toLowerCase();
}

function historyRangeMs(range){
  return range === "24h" ? 24*60*60*1000 :
    range === "30d" ? 30*24*60*60*1000 :
    7*24*60*60*1000;
}

function filteredModalHistory(){
  const cutoff = Date.now()-historyRangeMs(modalHistoryRange);
  return modalHistoryPoints.filter(point=>new Date(point.time).getTime()>=cutoff);
}

function renderModalHistory(){
  const canvas = byId("modalHistoryChart");
  const status = byId("modalHistoryStatus");
  if(!canvas || !window.Chart) return;

  const points = filteredModalHistory();
  modalHistoryChart?.destroy();

  if(points.length < 2){
    if(status) status.textContent = "Not enough stored history for this range yet.";
    modalHistoryChart = null;
    return;
  }

  const t = chartTheme();
  applyTopChartDefaults();
  modalHistoryChart = new Chart(canvas,{
    type:"line",
    data:{
      labels:points.map(point=>new Date(point.time)),
      datasets:[
        {
          label:"Members",
          data:points.map(point=>point.members),
          borderColor:t.diamond,
          backgroundColor:"rgba(88,229,238,.18)",
          fill:true,
          tension:.28,
          pointRadius:0,
          yAxisID:"members"
        ...
        {
          label:"Online",
          data:points.map(point=>point.online),
          borderColor:t.lapis,
    ...
    options:{
      ...
      scales:{
        x:{
          ...
          grid:{display:false},
          ticks:{maxTicksLimit:7,color:t.text,font:{weight:"700"}}
        },
        members:{position:"left",beginAtZero:false,grid:{color:t.grid},ticks:{color:t.text,font:{weight:"700"}}},
        online:{position:"right",beginAtZero:false,grid:{display:false}}
      }
    }
  });

  if(status){
    status.textContent = `${points.length.toLocaleString()} stored readings · ${new Date(points[0].time).toLocaleDateString()} to ${new Date(points.at(-1).time).toLocaleString()}`;
  }
}

async function loadModalHistory(server){
  const status = byId("modalHistoryStatus");
  modalHistoryPoints = [];
  modalHistoryChart?.destroy();
  modalHistoryChart = null;
  if(status) status.textContent = "Loading stored history…";

  try{
    const key = encodeURIComponent(topHistoryKey(server));
    const response = await fetch(`${API_BASE}/top-server-history?key=${key}`,{cache:"no-store"});
    const data = await response.json();
    if(!response.ok) throw new Error(data.error || `History request failed (${response.status})`);
    modalHistoryPoints = Array.isArray(data.history) ? data.history : [];
    renderModalHistory();
  }catch(error){
    if(status) status.textContent = error.message || "No stored history is available yet.";
  }
}
...
function render(data) {
  ...
  data.featuredServer = data.featuredServer ? {
    ...data.featuredServer,
    isFeaturedMinecraft: true,
    code: data.featuredServer.code || "minecraft",
    name: data.featuredServer.name || "Minecraft"
  } : null;
...
function compareServersAvailable() {
  const featured = payload?.featuredServer ? [payload.featuredServer] : [];
  const ranked = Array.isArray(payload?.servers) ? payload.servers : [];
  const seen = new Set();

  return [...featured, ...ranked].filter(server => {
...
function isFeaturedMinecraft(server) {
  return Boolean(
    server?.isFeaturedMinecraft ||
    String(server?.code || "").toLowerCase() === "minecraft" ||
    stableServerKey(server) === stableServerKey(payload?.featuredServer)
  );
}
...
function renderComparePicker() {
  ...
  picker.innerHTML = compareServersAvailable().map(server=>{
    ...
    const featured = isFeaturedMinecraft(server);
    return `<label class="compare-server-option ${checked ? "selected" : ""} ${featured ? "featured-compare-option" : ""}">
      <input type="checkbox" value="${key}" ${checked ? "checked" : ""}>
      ${server.iconUrl ? `<img src="${server.iconUrl}" alt="">` : `<span class="compare-server-letter">${String(server.name || "?").slice(0,1)}</span>`}
      <span>
        <strong>${featured ? "★ " : ""}${server.name}</strong>
        <small>${featured ? "Featured Minecraft · " : ""}${fmt(server.members)} members</small>
      </span>
      <b>${checked ? "✓" : "+"}</b>
    </label>`;
  }).join("");
...
function renderCustomComparison() {
  ...
  byId("compareSelectionSummary").innerHTML = selected.length
    ? selected.map(server=>`<span class="compare-chip ${isFeaturedMinecraft(server) ? "featured-minecraft-chip" : ""}">${server.iconUrl ? `<img src="${server.iconUrl}" alt="">` : ""}${isFeaturedMinecraft(server) ? "★ " : ""}${server.name}</span>`).join("")
  ...
  customCompareChart = new Chart(canvas,{
    ...
    data:{
      ...
      datasets:[{
        ...
        borderWidth:1,
        borderRadius:12,
        backgroundColor:selected.map((_,index)=>index===0 ? "rgba(99,216,255,.72)" : "rgba(78,143,255,.62)"),
        borderColor:selected.map((_,index)=>index===0 ? t.beacon : t.diamond)
      }]
    },
    options:{
      maintainAspectRatio:false,
...
byId("compareFeaturedMinecraft")?.addEventListener("click",()=>{
  const server = payload?.featuredServer;
  if(!server) return;
  const key = stableServerKey(server);
  if(!selectedCompareKeys.includes(key)){
    if(selectedCompareKeys.length >= 4) selectedCompareKeys.pop();
    selectedCompareKeys.unshift(key);
  }
  syncCompareUI();
  renderCustomComparison();
  openCompareDrawer();
...
function syncCompareUI(){
  const featuredButton = byId("compareFeaturedMinecraft");
  if(featuredButton && payload?.featuredServer){
    const featuredSelected = selectedCompareKeys.includes(stableServerKey(payload.featuredServer));
    featuredButton.classList.toggle("active", featuredSelected);
    featuredButton.textContent = featuredSelected ? "✓ Minecraft added" : "+ Compare Minecraft";
  }
  ...
  byId("stickyCompareServers").innerHTML = selected.map(server=>`
    <span class="sticky-server-chip ${isFeaturedMinecraft(server) ? "featured-minecraft-chip" : ""}">
      ${server.iconUrl ? `<img src="${server.iconUrl}" alt="">` : ""}
      ${isFeaturedMinecraft(server) ? "★ " : ""}${server.name}
    </span>
  `).join("");
}
...
function openEnhancedProfile(key){
  const topTenServers = Array.isArray(payload?.servers) ? payload.servers.slice(0,10) : [];
  const server = topTenServers.find(item=>serverProfileKey(item)===key);
  if(!server) return;
  ...
  if(details){
    details.insertAdjacentHTML("afterbegin",`
      <div><span>1-hour growth</span><strong>${server.growth1h == null ? "—" : signed(server.growth1h)}</strong></div>
      <div><span>7-day growth</span><strong>${server.growth7d == null ? "—" : signed(server.growth7d)}</strong></div>
      <div><span>30-day growth</span><strong>${server.growth30d == null ? "—" : signed(server.growth30d)}</strong></div>
      <div><span>Rank movement</span><strong>${server.rankChange == null ? "New history" : server.rankChange > 0 ? `↑ ${server.rankChange}` : server.rankChange < 0 ? `↓ ${Math.abs(server.rankChange)}` : "No change"}</strong></div>
      <div><span>First seen</span><strong>${server.firstSeenAt ? new Date(server.firstSeenAt).toLocaleString() : "—"}</strong></div>
...
byId("modalHistoryRanges")?.addEventListener("click",event=>{
  const button = event.target.closest("[data-history-range]");
  if(!button) return;
  modalHistoryRange = button.dataset.historyRange;
  document.querySelectorAll("[data-history-range]").forEach(item=>item.classList.toggle("active",item===button));
  renderModalHistory();
});


// Discord hero status mirrors the latest successful page refresh.
(function(){
  ...
  const observer = new MutationObserver(()=>{
    ...
    if(text) heroUpdated.textContent = text;
  });
  observer.observe(document.body,{subtree:true,childList:true,characterData:true});
  setTimeout(()=>{
    if(heroUpdated.textContent.includes("Waiting")){
      heroUpdated.textContent = "Live data updates automatically";
    }
  },2500);
})();


function reinforceTopChartContrast(){
  ...
  Object.values(Chart.instances).forEach(chartInstance=>{
    ...
    for(const axis of Object.values(scales)){
      ...
      axis.ticks = {...(axis.ticks||{}), color:t.text, font:{...((axis.ticks||{}).font||{}),weight:"700"}};
    }
    ...
    chartInstance.options.plugins.legend.labels = {
      ...(chartInstance.options.plugins.legend.labels||{}),
      color:t.text,
      font:{...((chartInstance.options.plugins.legend.labels||{}).font||{}),weight:"700"}
    };
    for(const dataset of chartInstance.data?.datasets || []){
      dataset.borderWidth = Math.max(Number(dataset.borderWidth || 0),3);
      dataset.pointHoverRadius = Math.max(Number(dataset.pointHoverRadius || 0),5);
    }
    chartInstance.update("none");
  });
}
setTimeout(reinforceTopChartContrast,300);
setTimeout(reinforceTopChartContrast,1200);
