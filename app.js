...
function renderMessageActivity(payload) {
  ...
  makeChart("messageActivityChart", {
    ...
    data:{datasets:[{
      ...
      data,
      borderColor:"#ffd866",
      backgroundColor:"rgba(255,216,102,.20)",
      fill:true,
      tension:.25,
...
function render(payload) {
  ...
  makeChart("memberChart", {
    type:"line",
    data:{datasets:[{label:"Members",data:chartData(history,"members"),borderColor:"#65f0a4",backgroundColor:"rgba(101,240,164,.18)",fill:true,tension:.22,spanGaps:false,pointRadius:history.length > 70 ? 0 : 2,pointHoverRadius:5}]},
  ...
  makeChart("onlineTrendChart", {
    type:"line",
    data:{datasets:[{label:"Online",data:chartData(history,"online"),borderColor:"#58e5ee",backgroundColor:"rgba(88,229,238,.18)",fill:true,tension:.22,spanGaps:false,pointRadius:history.length > 70 ? 0 : 2,pointHoverRadius:5}]},
  ...
  makeChart("onlineShareChart", {type:"doughnut",data:{labels:["Online","Offline"],datasets:[{data:[latest.online,Math.max(0,latest.members-latest.online)],backgroundColor:["#58e5ee","#263d39"],borderWidth:4,borderColor:"#131916"}]},options:{responsive:true,maintainAspectRatio:false,cutout:"68%",plugins:{legend:{position:"bottom",labels:{boxWidth:8,usePointStyle:true,font:{size:11}}},tooltip:{callbacks:{label:ctx=>`${ctx.label}: ${fmt(ctx.raw)} (${(ctx.raw/latest.members*100).toFixed(2)}%)`}}}}});
  ...
  makeChart("hourChart", {type:"bar",data:{labels:hours.map(p=>`${String(p.hour).padStart(2,"0")}:00`),datasets:[{data:hours.map(p=>p.change),backgroundColor:hours.map(p=>p.change<0?"#ff7a72":"#65f0a4"),borderRadius:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>`Net change: ${signed(ctx.raw)}`}}},scales:{x:{grid:{display:false},ticks:{maxTicksLimit:8}},y:{beginAtZero:true,ticks:{precision:0},grid:{color:document.documentElement.dataset.theme==="light"?"rgba(36,105,92,.18)":"rgba(154,224,206,.20)"}}}}});
