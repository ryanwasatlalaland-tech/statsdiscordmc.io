  ...
  makeChart("messageActivityChart", {
    ...
    options:{
      ...
      scales:{
        ...
        y:{suggestedMin:0,suggestedMax:100,ticks:{precision:0},grid:{color:document.documentElement.dataset.theme==="light"?"rgba(36,105,92,.18)":"rgba(154,224,206,.20)"}}
      }
    }
  });
}
...
function applyChartTheme(){
  ...
  Chart.defaults.color = light ? "#244f45" : "#e8fff5";
  Chart.defaults.borderColor = light ? "rgba(36,105,92,.20)" : "rgba(154,224,206,.22)";
  Chart.defaults.font.family = "DM Sans";
  Chart.defaults.font.size = 12;
  Chart.defaults.plugins.legend.labels.color = light ? "#244f45" : "#effff8";
  Chart.defaults.plugins.legend.labels.padding = 16;
  Chart.defaults.plugins.tooltip.backgroundColor = light ? "#173c35" : "#071714";
  Chart.defaults.plugins.tooltip.titleColor = "#ffffff";
  Chart.defaults.plugins.tooltip.bodyColor = "#effff8";
  Chart.defaults.plugins.tooltip.borderColor = light ? "#3c8a78" : "#58d6b1";
  Chart.defaults.plugins.tooltip.borderWidth = 1;
  Chart.defaults.plugins.tooltip.padding = 11;
}
applyChartTheme();

function chartPaletteForCanvas(canvas){
  const lightTheme = document.documentElement.dataset.theme === "light";
  const parchment = Boolean(canvas?.closest(".map-panel, .book-panel"));
  if(parchment){
    return {
      text:"#211a11",
      muted:"#4d412f",
      grid:"rgba(55,40,22,.26)",
      tooltipBg:"#2d251a",
      tooltipText:"#fff8e7"
    };
  }
  return {
    text:lightTheme ? "#244f45" : "#e8fff5",
    muted:lightTheme ? "#335f54" : "#cde9dc",
    grid:lightTheme ? "rgba(36,105,92,.20)" : "rgba(154,224,206,.22)",
    tooltipBg:lightTheme ? "#173c35" : "#071714",
    tooltipText:"#effff8"
  };
}

function readableScales(canvas, extra = {}){
  const p = chartPaletteForCanvas(canvas);
  return {
    x:{
      grid:{color:p.grid},
      ticks:{color:p.text,font:{weight:"700"}},
      title:{color:p.text,font:{weight:"800"}},
      ...extra.x
    },
    y:{
      grid:{color:p.grid},
...
function readablePlugins(canvas, extra = {}){
  const p = chartPaletteForCanvas(canvas);
  return {
    legend:{
      labels:{
...
function makeChart(id, config) {
  ...
  const palette = chartPaletteForCanvas(canvas);
  ...
  for (const axis of Object.values(scales)) {
    axis.grid = {
      ...(axis.grid || {}),
      color: axis.grid?.display === false ? axis.grid.color : palette.grid
    };
    axis.ticks = {
      ...(axis.ticks || {}),
      color: palette.text,
    ...
    if (axis.title) {
      axis.title = {
        ...axis.title,
        color: palette.text,
        font: {...(axis.title.font || {}), weight: "800"}
      };
    }
  }
...
function render(payload) {
  ...
  makeChart("memberChart", {
    type:"line",
    data:{datasets:[{label:"Members",data:chartData(history,"members"),borderColor:"#007a46",backgroundColor:"rgba(20,150,88,.18)",fill:true,tension:.22,spanGaps:false,pointRadius:history.length > 70 ? 0 : 2,pointHoverRadius:5}]},
    options:{responsive:true,maintainAspectRatio:false,interaction:{intersect:false,mode:"nearest"},plugins:{legend:{display:false},tooltip:{callbacks:{title:items=>items[0]?.raw?.source?.gap ? "Collection gap" : new Date(items[0].raw.x).toLocaleString(),label:ctx=>pointTooltip(ctx,"Members")}}},scales:{x:timeScaleOptions(),y:{beginAtZero:false,ticks:{callback:value=>fmt(value)},grid:{color:document.documentElement.dataset.theme==="light"?"rgba(36,105,92,.18)":"rgba(154,224,206,.20)"}}}}
  });

  makeChart("onlineTrendChart", {
    type:"line",
    data:{datasets:[{label:"Online",data:chartData(history,"online"),borderColor:"#007f91",backgroundColor:"rgba(0,127,145,.16)",fill:true,tension:.22,spanGaps:false,pointRadius:history.length > 70 ? 0 : 2,pointHoverRadius:5}]},
    options:{responsive:true,maintainAspectRatio:false,interaction:{intersect:false,mode:"nearest"},plugins:{legend:{display:false},tooltip:{callbacks:{title:items=>items[0]?.raw?.source?.gap ? "Collection gap" : new Date(items[0].raw.x).toLocaleString(),label:ctx=>pointTooltip(ctx,"Online")}}},scales:{x:timeScaleOptions(),y:{beginAtZero:false,ticks:{callback:value=>fmt(value)},grid:{color:document.documentElement.dataset.theme==="light"?"rgba(36,105,92,.18)":"rgba(154,224,206,.20)"}}}}
  });

  makeChart("onlineShareChart", {type:"doughnut",data:{labels:["Online","Offline"],datasets:[{data:[latest.online,Math.max(0,latest.members-latest.online)],backgroundColor:["#00dff0","#263d39"],borderWidth:4,borderColor:"#131916"}]},options:{responsive:true,maintainAspectRatio:false,cutout:"68%",plugins:{legend:{position:"bottom",labels:{boxWidth:8,usePointStyle:true,font:{size:11}}},tooltip:{callbacks:{label:ctx=>`${ctx.label}: ${fmt(ctx.raw)} (${(ctx.raw/latest.members*100).toFixed(2)}%)`}}}}});
  ...
  makeChart("hourChart", {type:"bar",data:{labels:hours.map(p=>`${String(p.hour).padStart(2,"0")}:00`),datasets:[{data:hours.map(p=>p.change),backgroundColor:hours.map(p=>p.change<0?"#ff5c55":"#20e88a"),borderRadius:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>`Net change: ${signed(ctx.raw)}`}}},scales:{x:{grid:{display:false},ticks:{maxTicksLimit:8}},y:{beginAtZero:true,ticks:{precision:0},grid:{color:document.documentElement.dataset.theme==="light"?"rgba(36,105,92,.18)":"rgba(154,224,206,.20)"}}}}});
...
function applyReadableChartOptions(chartInstance){
  ...
  for(const axis of Object.values(scales)){
    ...
    axis.ticks = {...(axis.ticks||{}), color:palette.text, font:{...((axis.ticks||{}).font||{}), weight:"700"}};
    if(axis.title) axis.title = {...axis.title, color:palette.text, font:{...(axis.title.font||{}), weight:"800"}};
  }
  ...
  for(const dataset of chartInstance.data?.datasets || []){
    ...
    dataset.pointHoverRadius = Math.max(Number(dataset.pointHoverRadius || 0), 5);
    dataset.pointBorderWidth = Math.max(Number(dataset.pointBorderWidth || 0), 2);
  }
  chartInstance.update("none");
}

function refreshAllChartContrast(){
  if(!window.Chart?.instances) return;
  Object.values(Chart.instances).forEach(applyReadableChartOptions);
}

setTimeout(refreshAllChartContrast, 300);
setTimeout(refreshAllChartContrast, 1200);
